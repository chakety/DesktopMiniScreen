...
void ds_nvs_save_wifi_info(){
    ...
    //NVS open successful, writing the wifi flag, ssid, and pwd.
...
NVS_WIFI_INFO_E ds_nvs_read_wifi_info(){
    ...
    //Saving an 8-bit in wifi_config for saving wifi config
    ...
    if(wifi_config_flag == NVS_WIFI_INFO_SAVE){
        ...
        size_t psw_len = sizeof(pwd);
        ESP_ERROR_CHECK(nvs_get_str(nvs_handle, "ssid", ssid, &ssid_len));
        ESP_ERROR_CHECK(nvs_get_str(nvs_handle, "psw", pwd, &psw_len));
        set_system_data_wifi_info(ssid,ssid_len, pwd,psw_len);
